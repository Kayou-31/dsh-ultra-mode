...
type ReRpcResult<T> = { ok: true; value: T } | { ok: false; error: { code: string; message: string } }
...
function drawRadiation(
  ...
  for (let x = 0; x < origin; x += cell) {
    ...
    if (columnEnergy > 0.012) {
      ...
      const red = isDark ? Math.round(52 + 128 * nearness + 70 * wave) : Math.round(30 + 62 * nearness + 16 * wave)
      const green = isDark ? Math.round(60 + 60 * nearness + 42 * crest) : Math.round(92 + 74 * nearness + 28 * crest)
      const blue = isDark ? Math.round(176 + 70 * nearness + 8 * wave) : Math.round(184 + 60 * nearness)
      const alpha = isDark ? Math.min(0.88, columnEnergy * 0.72) : Math.min(0.6, columnEnergy * 0.52)
    ...
    for (let y = 0; y < height; y += cell) {
      ...
      const red = isDark ? Math.round(60 + 148 * hot + 40 * wave + 34 * crest) : Math.round(30 + 74 * hot + 12 * wave)
      const green = isDark ? Math.round(72 + 80 * hot + 44 * crest) : Math.round(100 + 72 * hot + 22 * crest)
      const blue = isDark ? Math.round(188 + 62 * hot) : Math.round(196 + 54 * hot)
      context.fillStyle = `rgba(${red},${green},${blue},${isDark ? alpha : alpha * 0.7})`
  ...
  glow.addColorStop(0.14, isDark ? 'rgba(190,192,255,.54)' : 'rgba(168,212,255,.48)')
  glow.addColorStop(0.44, isDark ? 'rgba(108,76,255,.28)' : 'rgba(42,116,208,.22)')
...
function UltraSlider({ sessionId, rpc }: { sessionId?: string; rpc: HostRpc }): React.ReactElement {
  ...
  const onPointerUp = (event: React.PointerEvent<HTMLInputElement>): void => {
    if (!draggingRef.current) return
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        'span',
        { className: 'ult-fx', 'aria-hidden': true },
        React.createElement('canvas', { ref: canvasRef, className: 'ult-canvas' }),
        React.createElement('span', { className: 'ult-flare' }),
      ),
      ...
      React.createElement('span', { className: 'ult-knob', 'aria-hidden': true }),
    ),
  )
}
